classdef ExperimentRunner < handle
    ...
    methods
        ...
        function plot_comparison(obj)
            ...
            subplot(1,3,1);
            bar(t_convs); set(gca,'XTickLabel', names, 'XTickLabelRotation', 30);
            ylabel('Convergence Time (s)'); title('Convergence'); grid on;

            subplot(1,3,2);
            bar(energies); set(gca,'XTickLabel', names, 'XTickLabelRotation', 30);
            ylabel('Total Energy (J)'); title('Energy'); grid on;

            subplot(1,3,3);
            bar(conn_rats*100); set(gca,'XTickLabel', names, 'XTickLabelRotation', 30);
